...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (status) {
      ...
      if (tracking.referenceType === "product_order") {
        const updateData: Record<string, any> = {
          status: status, // Now directly using tracking status on the Order
        }

        // Also update estimated delivery date if provided
        if (estimatedDeliveryDate) {
          updateData.estimatedDeliveryDate = new Date(estimatedDeliveryDate)
        }

        await Order.findByIdAndUpdate(tracking.referenceId, updateData)
      } else if (tracking.referenceType === "home_test") {
    ...
    if (estimatedDeliveryDate && tracking.referenceType === "product_order") {
      await Order.findByIdAndUpdate(tracking.referenceId, {

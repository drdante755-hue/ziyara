...
type TokenShape = {
  ...
  exp?: number
  iat?: number
  profileCompleted?: boolean // New field for profile completion
}
...
export default async function middleware(req: NextRequest) {
  ...
  if (pathname.startsWith(USER_PREFIX)) {
    ...
    if (!token.emailVerified && pathname !== "/user/user-info") {
    ...
    if (isUserHome && !token.profileCompleted) {
      return NextResponse.redirect(new URL("/user/user-info", req.url))
    }

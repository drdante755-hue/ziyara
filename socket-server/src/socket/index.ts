...
const ticketRooms = new Map<string, Map<string, ActiveUser>>()
 ...
 */
const getTicketRoom = (ticketId: string): string => `ticket_${ticketId}`

/**
 * Validate and convert string to ObjectId
 */
const toObjectId = (value: string | undefined | null, fieldName: string): mongoose.Types.ObjectId => {
 ...
 */
export const initializeSocket = (io: Server<ClientToServerEvents, ServerToClientEvents>): void => {
  io.on("connection", (socket: Socket<ClientToServerEvents, ServerToClientEvents>) => {
     ...
     */
    socket.on("join_ticket", async (payload: JoinTicketPayload) => {
      try {
     ...
     */
    socket.on("leave_ticket", (payload: LeaveTicketPayload) => {
    ...
    socket.on("send_message", async (payload: SendMessagePayload) => {
      try {
        ...
        if (ticketStatus === TicketStatus.CLOSED || ticketStatus === "resolved") {
          socket.emit("error", { 
     ...
     */
    socket.on("ticket_status_change", async (payload: TicketStatusChangePayload) => {
      try {
        ...
        if (status === TicketStatus.CLOSED || (status as string) === "closed") {
        ...
        const ticket = await Ticket.findByIdAndUpdate(ticketObjectId, { status, updatedAt: new Date() }, { new: true })
     ...
     */
    socket.on("assign_agent", async (payload: AssignAgentPayload) => {
      try {
    ...
    socket.on("mark_as_read", async (payload: MarkAsReadPayload) => {
      try {
        ...
        await Ticket.findByIdAndUpdate(ticketObjectId, { unreadCount: 0 })
    ...
    socket.on("disconnect", (reason) => {
      ...
      console.log(`[Socket] Remaining connected clients: ${io.sockets.sockets.size}`)
...
function getStatusLabel(status: TicketStatus): string {
  const labels: Record<TicketStatus, string> = {
    ...
    [TicketStatus.PENDING]: "قيد المعالجة",
 ...
 */
export const getTicketActiveUsers = (ticketId: string): ActiveUser[] => {
  const roomUsers = ticketRooms.get(ticketId)
 ...
 */
export const isUserInTicket = (ticketId: string, userId: string): boolean => {
  const roomUsers = ticketRooms.get(ticketId)
